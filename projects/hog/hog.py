...
def picky_piggy(score):
    ...
    if score == 0:
        ...
        if n == 0:
            return 7
    return one_serverth // pow(10, 6 - n) % 10
    # END PROBLEM 2
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1):
        say_running_high = running_high
        say_last_score = last_score
        if who == 0:
            if  score0 > say_last_score:
                if score0 - say_last_score > say_running_high:
                    say_running_high = score0 - last_score
                    print(score0 - say_last_score, "point(s)! That's a record gain for Player 0!")
                say_last_score = score0
        else:
            if  score1 > say_last_score :
                if score1 - say_last_score > say_running_high:
                    say_running_high = score1 - say_last_score
                    print(score1 - say_last_score, "point(s)! That's a record gain for Player 1!")
            say_last_score = score1
        return announce_highest(who, say_last_score, say_running_high)
    return say
    # END PROBLEM 7
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if picky_piggy_strategy(score, opponent_score, cutoff, num_rolls) == 0:
       return 0 
    will_get_score = picky_piggy(opponent_score)
    if hog_pile(score + will_get_score, opponent_score):
        return 0
