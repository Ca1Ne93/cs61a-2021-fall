...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def check(lst):
        list = split(remove_punctuation(lst))
        for x in list:
            for y in topic:
                if lower(x) == y:
                    return True
...
def accuracy(typed, reference):
    ...
    if typed == '' and reference == '':
        return 100.0
    elif typed == '' and len(reference) != 0 :
        return 0.0
    elif len(typed) == 0 and reference == '':
        return 0.0
    correct_n = 0
    ...
    return correct_n / len(typed) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    list = []
    for string in valid_words:

        if typed_word == string :
            return typed_word
        else:
            list += [[diff_function(typed_word, string, limit), string]]
        
    if list == []:
        return typed_word
    
    #print('DEBUG:', "typed_word:", typed_word, list)
       
    pair =  min(list, key = lambda x: x[0])
    if pair[0] > limit:
        return typed_word
    return pair[1]
    # END PROBLEM 5
...
def final_diff(start, goal, limit):
    ...
    if start == '' or goal == '' :  
        ...
        if goal[1] != start[1] and goal[0] == goal[1]:
            return 1 + minimum_mewtations(start[1:], goal[2:], limit - 1)
        return minimum_mewtations(start[1:], goal[1:], limit)
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    ret = []
    for sList in times_per_player:
        time_list = []
        for i in range(1,len(sList)):
            time_list += [sList[i] - sList[i - 1]]
        ret += [time_list]
    
    return match(words, ret)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for i in word_indices:
        time_min = get_times(match)[0][i]
        player_min = 0
        for player in player_indices[1:] :
            if get_times(match)[player][i] < time_min:
                time_min = get_times(match)[player][i]
                player_min = player
        min_list += [player_min]
    ...
    for i in word_indices:
        list[min_list[i]] += [get_words(match)[i]]
