...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        range = 0
        while place and not place.is_hive and range <= self.max_range:
            if len(place.bees) and range >= self.min_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in now_place.bees[:]:   ## [:] 使用副本遍历
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if len(self.place.bees) and self.chew_countdown == 0:
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        for bee in self.place.bees[:]:   ## [:] 使用副本遍历
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        print("DEBUG:", self.has_scary, self.scary_length)
        if self.has_scary and self.scary_length > 0:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        now_place = self.place.entrance
        length = 0
        dict = {}
        
        if self.place.ant.is_container:
            dict[self.place.ant] = length
        ...
        while now_place and not now_place.is_hive:
            # ant
            if now_place.ant:
                dict[now_place.ant] = length
                if now_place.ant.is_container:
                    dict[now_place.ant.ant_contained] = length
            # bee
            if len(now_place.bees):
