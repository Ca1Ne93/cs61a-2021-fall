...
def totals_tree(m):
    ...
    right_size = total_weight(end(right(m)))
    root = left_size + right_size
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    for branch in branches(t):
        folder.append(replace_loki_at_leaf(branch, lokis_replacement)) 
    return tree(label(t), folder)
...
def div_interval(x, y):
    ...
    lower_y = lower_bound(y)
    upper_y = upper_bound(y)
    assert upper_y != 0 or lower_y != 0, 'AssertionError'
    lower = 1 / upper_bound(y)
    upper = 1 / lower_bound(y)
    assert lower <= upper, 'AssertionError'
    reciprocal_y = interval(lower, upper)
