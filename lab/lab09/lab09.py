...
class Account:
    ...
    def deposit(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.balance += amount
        self.transactions.extend([('deposit', self.balance)])
        self.deposits += 1
        return self.balance
    ...
    def withdraw(self, amount):
        ...
        if amount <= self.balance:
            self.balance -= amount
        self.transactions.extend([('withdraw', amount)])
        self.withdraws += 1
        return self.balance
...
def shuffle(cards):
    ...
    half = cards[:len(cards) >> 1]
    shuffled = []
    for i in range(len(half)):
        print("DEBUG:", i, half[i])
...
def insert(link, value, index):
    ...
    while cur_link is not Link.empty:
        if index == 0:
            tmp = cur_link.first
            cur_link.first = value
            cur_link.rest = Link(tmp, cur_link.rest)
            return
        else:
            index -= 1
