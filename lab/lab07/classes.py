...
class Player:
    def __init__(self, deck, name):
        ...
        "*** YOUR CODE HERE ***"
        self.hand = []
        while len(self.hand) < 5 and not self.deck.is_empty():
...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        while len(opponent.hand) > 2:
           opponent.hand.pop(random.randrange(len(opponent.hand)))
        
        while len(opponent.hand) < 5:
            opponent.draw()
...
# An Instructor card will add the opponent card's attack and defense to all cards in their 
# deck and then remove all cards in the opponent's deck that share its attack or defense!
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for players in [player, opponent]:
            for card in players.deck.cards:
                card.attack += opponent_card.attack
                card.defense += opponent_card.defense
        opponent.deck.cards = [_ for _ in opponent.deck.cards if _.attack != _.defense]
